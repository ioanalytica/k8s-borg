...
### 8.7 Workstream register (state 2026-09-09)
...
| 2b | `--stats -v` on every Borg 2 compact, parse and persist the five values, refresh `total_size` with source `compact_stats` | borg-ui backend + agent | agent half MERGED with #968 (agent 0.1.4 runs `compact --stats`); server half = upstream #974, open, CodeRabbit approved, mergeable, rebuilt on `operations.result` after phase 5 (#947), no migration; #931 stays open until it merges | - | S |
...
| 6 | `RepositoryStatus` model: backup from archives, prune from removals, compact/check from jobs, one stats component, cadence-based overdue, single writer of `last_backup` | borg-ui backend + frontend | backend MERGED upstream #967 (2026-09-08, `repository_status`, `/status`); the strip itself retired by #975 (2026-09-09) in favour of `Last Prune` / `Last Index` in the card metadata row; #935 closed. OPEN: the single stats component for card, viewer header and info dialog (with W13), filed as karanhudia/borg-ui#981 (2026-09-09) | 2 and 5 for size/last_modified only | L |
...
| 8 | `repo-info --stats [--json]` read-only from the chunk index | borgbackup | borgbackup/borg#10329 open; maintainer open to a non-default option, warns about latency and O(archives) (2026-09-05). borgbackup/borg#10335 (`create --json` deduplicated size) CLOSED COMPLETED | - | upstream |
| 9 | Pin bump, tag `1.1.7-alpha.3`, roll out both images (agents 0.1.4 + server) | k8s-borg | DONE: chart 1.1.7-alpha.7 (k8s-borg 475ecd9) on borg-ui io/integration `1ac2760a` = upstream/main f94b048c + #974; both clusters on it since 2026-09-09 05:50Z (same UI digest, Alembic head `f7a8b9c0d1e2`, `compact_jobs.stats` gone, `total_size_source` / `borg_last_modified` present) | 1-5 | S |
...
| 13 | Archive viewer header (`RepositoryStatsGrid` via `useRepositoryStats`) shows `Repository Size = 0 B` on Borg 2: it reads `rinfo_stats.unique_csize` from the live `repo-info` instead of `repositories.total_size` + `total_size_source`, which W2/W2b already fill (observed 2026-09-06, 32 archives, 11.46 GB archive size, 0 B repository size). Fold into the W6 "one stats component": header, card and info dialog read the status model's size with its source label, never a per-version borg-output field; no size = "unknown", never `0 B` | borg-ui frontend (+ `/repositories/{id}/info` shape) | issue karanhudia/borg-ui#981 posted 2026-09-09 (one stats component; screenshots on fork branch `assets/issue-stats-component-2026-09-09`) | 2, 6 | S |
| 14 | Archive history on agent repositories: `history_index` has no agent job and marks every archive `skipped` / `agent_diff_unsupported` (observed 2026-09-07: all archives of every agent repository `skipped`, every `history_index` operation skipped with that reason, trigger `followup` and `reconcile`). The Changes tab shows the generic "History indexing was skipped" with a "Rebuild the history index" link that resets to `pending` and skips again; the Files tab's path history reads `archive_changes` only and asserts "No earlier archive contains this path." from an index that was never built. Fix: persist `history_skip_reason` on `archives`, expose `history_capability` (available / plan_locked / agent_unsupported) on list, detail and hub, drop `history_index` from the chains and `reindex` for agent repositories (like `PLAN_GATED_KINDS`), `path_history` returns coverage (indexed / total), UI shows one message per reason, no rebuild control and no history stage where it cannot run, file panel says "not available" / "not indexed yet" / "n of m indexed" instead of a claim about the archives | borg-ui backend + frontend | issue karanhudia/borg-ui#953 open | overlaps 1 (dropping the stage removes the stats-after-skipped-history dependency for agent repositories) | M |
| 15 | Archive Info tab on Borg 2 shows `Compressed size = Unknown` and `Unique to this archive = Unknown` (observed 2026-09-07, m3s, b24, archive with 186.53 MB original). Measured: b24 `Statistics.as_dict` (the `--json` payload of both `create` and `info`) carries only `original_size`, `nfiles`, timings, `files_stats`, `store_stats`. `compressed_size` no longer exists in Borg 2 (csize dropped). `usize` (deduplicated size) is accumulated during `create` and printed only in the human `--stats` text ("Deduplicated size:"), not in JSON; `info` sets `usize = 0` ("expensive to compute"). So upstream #948 (`repository.archive_info` for agent repos) fills `original_size` and `nfiles` only; it cannot fill the other two. Today `backup_jobs` rows on Borg 2 carry `compressed_size = 0` and `deduplicated_size = 0` (written as 0, not NULL) and the archive row is not linked to its backup operation (`backup_operation_id` NULL for agent backups). Fix path: (a) UI: on Borg 2 the compressed tile says "not available on Borg 2" (hide tile and bar), never "Unknown"; (b) dedup size per archive from backup time: agent captures the `--stats` text block (or borgbackup feature request borgbackup/borg#10335, posted 2026-09-07: add `deduplicated_size` to the `create --json` output, cheap since `usize` is known there) and the server writes `archives.deduplicated_size` from the backup result via the archive/operation link; (c) backup stats: NULL instead of 0 for fields Borg 2 does not report ("never 0", as in W2/W6) | borg-ui backend + agent + frontend (+ borgbackup for the JSON field) | borgbackup/borg#10335 CLOSED COMPLETED (create --json reports the deduplicated size in a future Borg 2 beta); borg-ui part still open: agent parses `--stats` text or the JSON once shipped, plus the archive-to-backup-operation link | 4 (#948) for the fill path, 6 for the tile wording | M |
| 16 | Plan prune/compact fails "agent job could not be queued (dispatch failed)" when the backup follow-up chain (#949) is listing the repository: admission refuses a write beside active read work and `_fail_orphaned_maintenance_job` fails the `*_jobs` row at once instead of waiting like the runner (#948). Measured 8/685 prunes, 3/677 compacts in three days, first with the wave rollout; no data loss, next cycle catches up | borg-ui backend | issue karanhudia/borg-ui#982 posted 2026-09-09 | 7 (#949), 1 (#948) | S |
| 17 | Inline plan prune/compact (phase 5 `start_inline_maintenance`, created `running`) stays `running` forever when its agent job cannot be queued: `_fail_orphaned_maintenance_job` only knows the legacy `*_jobs` models, so the operation is never closed, and admission then refuses every backup of the repository until a restart. Seen twice on 2026-09-09 (trigger = the #982 race). Fix: resolve through `resolve_maintenance_job`, plan path reports the cause, runtime safety net for running exclusive operations without pid or agent job | borg-ui backend | issue karanhudia/borg-ui#983 posted 2026-09-09 | 16 (#982) | S |
