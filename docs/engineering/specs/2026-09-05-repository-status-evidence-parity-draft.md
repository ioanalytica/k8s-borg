...
| 6 | `RepositoryStatus` model: backup from archives, prune from removals, compact/check from jobs, one stats component, cadence-based overdue, single writer of `last_backup` | borg-ui backend + frontend | backend MERGED upstream #967 (2026-09-08, `repository_status`, `/status`); the strip itself retired by #975 (2026-09-09) in favour of `Last Prune` / `Last Index` in the card metadata row; #935 closed. OPEN: the single stats component for card, viewer header and info dialog (with W13) | 2 and 5 for size/last_modified only | L |
